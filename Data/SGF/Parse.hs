...
import Data.SGF.Types hiding (Game (..), GameInfo (..), GameNode (..), Move (..), Setup (..))
...
readRational s = liftM3 (\s n d -> s * (fromInteger n + d)) maybeSign maybeNum maybeDen
  where
...
move move = do
  ...
  case color_ of
    ...
    [black, white] ->
      let color = if black then Black else White
       in do
...
ruleSetOcti s = case break (== ':') s of
  ...
  (majorOrMinors, "") ->
    liftM (flip OctiRuleSet (Set.empty)) (majorVariation majorOrMinors)
